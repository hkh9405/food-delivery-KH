...
- 상점주는 요리시작때와 완료 시점에 시스템에 상태를 입력한다 -> 상점주는 요리 시작과 완료 예정시간을 설정하고 예정시간에 가장 가까운 라이더가 도착하도록 예약한다.
- 고객은 아직 요리가 시작되지 않은 주문은 취소할 수 있다
- 요리가 완료되면 고객의 지역 인근의 라이더들에 의해 배송건 조회가 가능하다 -> 예정시간 보다 빠르게 요리가 완료되면 직접 라이더를 조회하여 배송건 요청이 가능하다
- 라이더가 해당요리를 pick한 후, 앱을 통해 통보한다
- 고객이 주문상태를 중간중간 조회한다
- 주문상태가 바뀔 때 마다 카톡으로 알림을 보낸다
- 고객이 요리를 배달 받으면 배송확인 버튼을 탭하여, 모든 거래가 완료된다 -> 라이더가 배송이 완료되면 배송완료 버튼을 탭하여 모든 거래가 완료된다
